 ...
 *
 */
package org.summerboot.jexpress.infra.websocket.handler;
...
import org.summerboot.jexpress.api.websocket.WebSocketPipelinesInitializer;
import org.summerboot.jexpress.infra.netty.config.NioConfig;
import org.summerboot.jexpress.infra.websocket.stomp.StompToWebSocketTextFrameEncoder;
import org.summerboot.jexpress.infra.websocket.stomp.WebSocketFrameToByteBufDecoder;
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof FullHttpRequest) {
            ...
            for (String name : orderedNamedWebsocket) { // uriPredefined = "/ws/chat"
                String uriFormatted = name.endsWith("/") ? name : name + "/";
                if (uriRequestedWithOTT.startsWith(uriFormatted)) {
                    oneTimeToken = uriRequestedWithOTT.substring(uriFormatted.length());
                    uriRequested = name;
                    break;
                }
            }
