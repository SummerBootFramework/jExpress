 ...
 *
 */
package org.summerboot.jexpress.util.runtime;
    ...
    public static void RTO(int code, String msg, Throwable ex) {
        if (msg != null) {
            if (code == BootErrorCode.RTO_CLS_EXIT) {
                System.out.println(BootConstants.BR + BootConstants.BR + msg);
            } else {
                System.err.println(BootConstants.BR + BootConstants.BR + "\t Error#" + code + ": " + msg);
            }
        }
        if (ex != null) {
            ex.printStackTrace(System.err);
