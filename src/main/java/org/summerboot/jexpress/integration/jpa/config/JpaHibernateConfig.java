 ...
 *
 */
package org.summerboot.jexpress.integration.jpa.config;
...
abstract public class JpaHibernateConfig extends JpaConfig {
    ...
    protected void generateTemplate_ConnectionPool(StringBuilder sb, Properties currentValues) {
        sb.append("# Note: Maximum waiting time for a connection from the pool" + BootConstants.BR);
        appendCurrentValue("hibernate.hikari.connectionTimeout", currentValues, "30000", sb);

        sb.append("# Note: Minimum number of ideal connections in the pool" + BootConstants.BR);
        appendCurrentValue("hibernate.hikari.minimumIdle", currentValues, "10", sb);

        sb.append("# Note: Maximum number of actual connection in the pool" + BootConstants.BR);
        appendCurrentValue("hibernate.hikari.maximumPoolSize", currentValues, "10", sb);

        sb.append("# Note: Maximum time that a connection is allowed to sit ideal in the pool" + BootConstants.BR);
        ...
        sb.append("# Note: enables Java Management Extensions (JMX) MBeans for the HikariCP connection pool, " +
                ...
                "It is essential for tracking pool health but must be enabled explicitly as it defaults to false" + BootConstants.BR);
