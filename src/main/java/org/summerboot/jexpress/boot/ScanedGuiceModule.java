...
public class ScanedGuiceModule extends AbstractModule {
     ...
     */
    private final Map<Class, Map<String, List<BootDeployee.ServiceMetadata>>> scanedServiceBindingMap;
    private final Set<String> userSpecifiedImplTags;
    private final StringBuilder memo;

    public ScanedGuiceModule(Map<Class, Map<String, List<BootDeployee.ServiceMetadata>>> scanedServiceBindingMap, Set<String> userSpecifiedImplTags, StringBuilder memo) {
    ...
    public void configure() {
        ...
        for (Class interfaceClass : scanedServiceBindingMap.keySet()) {
            Map<String, List<BootDeployee.ServiceMetadata>> taggeServicedMap = scanedServiceBindingMap.get(interfaceClass);
            BootDeployee.ServiceMetadata defaultImpl = null;
            BootDeployee.ServiceMetadata tagMatchImpl = null;
            BootDeployee.ServiceMetadata bindingImpl = null;
            boolean needToBindOneToOne = false;
            for (String uniqueKey : taggeServicedMap.keySet()) {
                BootDeployee.ServiceMetadata serviceImpl = taggeServicedMap.get(uniqueKey).get(0);//validated by BootDeployee.scanAnnotation_Service_ValidateBindingMap() for error msg
                if (serviceImpl == null) {
                    continue;
                }
                ...
                if (isOnToOne) {//non-named: one(interface) <--> one(Impl)
                    ...
                    BootDeployee.ServiceMetadata namedDefaultImpl = null;
                    BootDeployee.ServiceMetadata namedTagMatchImpl = null;
                    if (Service.NO_TAG.equals(implTag)) {
                    ...
                    BootDeployee.ServiceMetadata namedBindingImpl = namedTagMatchImpl != null ? namedTagMatchImpl : namedDefaultImpl;
