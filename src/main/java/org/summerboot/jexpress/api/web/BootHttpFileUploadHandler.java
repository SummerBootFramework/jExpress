...
//NOT @Singleton
public abstract class BootHttpFileUploadHandler<T> extends SimpleChannelInboundHandler<HttpObject> {
    ...
    //protected static final Authenticator auth = new AuthenticatorImplLdap();
    static final AtomicLong COUNTER_UPLOAD_COUNT = new AtomicLong(0);
    ...
    protected void channelRead0(final ChannelHandlerContext ctx, final HttpObject httpObject) throws IOException {
        if (httpObject instanceof HttpRequest) {
            ...
            if (isMultipart) {
                ...
                hitIndex = COUNTER_UPLOAD_COUNT.incrementAndGet();
                context = SessionContext.build(BootConstants.APP_ID + "-upload" + hitIndex, hitIndex);
            ...
            if (httpDecoder != null) {
                try {
                    ...
                    if (isOverSized) {
                        ...
                        //SessionContext ioc = SessionContext.build(hitIndex);
    ...
    protected boolean onPartialChunk(ChannelHandlerContext ctx, long maxAllowedSize) {
...
//                    }
    ...
    protected void onLastChunk(ChannelHandlerContext ctx) {
