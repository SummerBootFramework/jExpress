...
This release introduces significant simplification and consolidation of the application lifecycle management APIs. Migration is required for projects using the deprecated interfaces.
...
* **`cfg_nio.properties`** added:
    * `nio.server.idle.threshold.second`: Defines the connection inactivity threshold (in seconds) for the NIO server.
* **`cfg_grpc.properties`** added:
    * `gRpc.server.idle.threshold.second`: Defines the connection inactivity threshold (in seconds) for the gRPC server.
...
* Log trace enable on `org.summerboot.jexpress.web.netty.handler.BootHttpRequestHandler` will override `@org.summerboot.jexpress.annotation.Log` settings to log all requests and responses.
